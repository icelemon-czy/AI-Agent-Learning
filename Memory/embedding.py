...
class DashScopeEmbedding(EmbeddingModel):
    ...
    行为：
    - 如提供 base_url，则优先使用 REST 接口（POST {base_url}/embeddings）。
...
def create_embedding_model_with_preferred_type(preferred_type: str = "dashscope", **kwargs) -> EmbeddingModel:
...
def _build_embedding_model()-> EmbeddingModel:
    ...
    return create_embedding_model_with_preferred_type(preferred_type=preferred, **kwargs)
...
# --- Demo: 使用 LocalTransformerEmbedding 和 TFIDFEmbedding ---
if __name__ == "__main__":
    print("=== LocalTransformerEmbedding Demo ===")
    # 创建嵌入模型实例
    embedder = LocalTransformerEmbedding()
    
    # 示例 1: 单个字符串
    single_text = "Hello, this is a test sentence."
    embedding_single = embedder.encode(single_text)
    print(f"单个文本嵌入维度: {len(embedding_single)}")
    
    # 示例 2: 多个字符串
    multiple_texts = ["First sentence.", "Second sentence.", "Third sentence."]
    embeddings_multiple = embedder.encode(multiple_texts)
    print(f"多个文本嵌入数量: {len(embeddings_multiple)}")
    print(f"每个嵌入维度: {len(embeddings_multiple[0])}")
    
    print(f"模型维度: {embedder.dimension}")
    print("✅ LocalTransformerEmbedding Demo 完成\n")
    
    print("=== TFIDFEmbedding Demo ===")
    # 创建 TF-IDF 嵌入实例
    tfidf_embedder = TFIDFEmbedding(max_features=100)
    ...
    print(f"TF-IDF 模型训练完成，词汇表大小: {len(tfidf_embedder._vectorizer.get_feature_names_out())}")
    
    # 示例 1: 单个字符串
    single_text_tfidf = "This is a new document."
    embedding_single_tfidf = tfidf_embedder.encode(single_text_tfidf)
    print(f"单个文本 TF-IDF 嵌入维度: {len(embedding_single_tfidf)}")
    
    # 示例 2: 多个字符串
    multiple_texts_tfidf = ["New first.", "New second.", "New third."]
    embeddings_multiple_tfidf = tfidf_embedder.encode(multiple_texts_tfidf)
    print(f"多个文本 TF-IDF 嵌入数量: {len(embeddings_multiple_tfidf)}")
    print(f"每个 TF-IDF 嵌入维度: {len(embeddings_multiple_tfidf[0])}")
    
    print(f"TF-IDF 模型维度: {tfidf_embedder.dimension}")
    print("✅ TFIDFEmbedding Demo 完成")